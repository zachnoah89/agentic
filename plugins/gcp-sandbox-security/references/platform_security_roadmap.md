...
* **The `ide-provisioner@` Exemption Paradox (`SEC-001` — Managed Web IDE Proxy Exfiltration)**:
  ...
  * Furthermore, Managed Web IDE Proxy containers run in Cloud Sandbox Platform' central GKE cluster (`sandbox-platform-prod`), **not** inside the student's VPC (`tf/secure_network.tf`), and Vertex AI (`https://aiplatform.googleapis.com`) is a Google Cloud Control-Plane API reachable from any IP on the public internet. Once `/home/user/keys.json` is copied out of the browser IDE, `tf/secure_network.tf` is 100% bypassed.
